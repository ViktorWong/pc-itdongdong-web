  ...
  jQuery.extend({
    Deferred: function(func) {
          ...
          then: function(/* fnDone, fnFail, fnProgress */) {
            ...
            return jQuery
              .Deferred(function(newDefer) {
                //thenæœ€å¤šæŽ¥å—3ä¸ªå‚æ•°ï¼Œå¯¹åº”ç€tuplesé‡Œé¢çš„3ä¸ªæ•°ç»„
                jQuery.each(tuples, function(i, tuple) {
                  // thenæ–¹æ³•ä¸­æ”¯æŒ3ä¸ªå‚æ•°fnDone, fnFail, fnProgress
                  var fn = jQuery.isFunction(fns[i]) && fns[i];
                  // deferred[ done | fail | progress ] for forwarding actions to newDefer
                  // deferred.done = list.add()
                  // deferred.fail = list.add()
                  // deferred.progress = list.add()
                  // ç»™å½“å‰çš„deferredå¯¹è±¡å¢žåŠ ä¸‰ä¸ªæ–¹æ³•ï¼Œç”¨æ¥å…³è”ä¸‹ä¸€ä¸ªdeferredå¯¹è±¡
                  deferred[tuple[1]](function() {
                    //æ‰§è¡Œç›‘æŽ§çš„å‡ ä¸ªæ–¹æ³•ï¼Œæ‹¿åˆ°è¿”å›žå€¼
                    var returned = fn && fn.apply(this, arguments);
                    //å¦‚æžœåå›žçš„åˆæ˜¯ä¸€ä¸ªå¼‚æ­¥
                    if (returned && jQuery.isFunction(returned.promise)) {
                      ...
                      //å€¼ç±»åž‹
                      //è§¦å‘ä¸‹ä¸€ä¸ªDeferredçš„ç®¡é“è®¾è®¡ä¸­çš„resolve reject notify
      ...
      jQuery.each(tuples, function(i, tuple) {
        ...
        // å¦‚æžœæœ‰çŠ¶æ€æŽ§åˆ¶
        if (stateString) {
          //å¢žåŠ ä¸‰ä¸ªé¢„å¤„ç†å‡½æ•°
          list.add(
            function() {
              // state = [ resolved | rejected ]
              // æ‰§è¡Œæ“ä½œæ—¶å€™ï¼Œä¿®æ”¹å¯¹åº”çš„çŠ¶æ€
              state = stateString;
              // [ reject_list | resolve_list ].disable; progress_list.lock
              //å¢žåŠ å¯¹åº”çš„æ–¹æ³•ï¼Œè®©æ“ä½œä¸å¯é€†
              //ç¦ç”¨å¯¹ç«‹çš„é‚£æ¡é˜Ÿåˆ—
              //æ³¨æ„ 0^1 = 1   1^1 = 0
              //å³æ˜¯æˆåŠŸçš„æ—¶å€™ï¼ŒæŠŠå¤±è´¥é‚£æ¡é˜Ÿåˆ—ç¦ç”¨
              //å³æ˜¯æˆåŠŸçš„æ—¶å€™ï¼ŒæŠŠæˆåŠŸé‚£æ¡é˜Ÿåˆ—ç¦ç”¨
      ...
      //è¿™ä¸€æ­¥ä¹‹å‰promiseå’Œdeferredç»‘å®šäº†ä»¥ä¸‹æ–¹æ³•
      // deferred[ resolve | reject | notify ]
      // deferred[ resolveWith | rejectWith | notifyWith ]
      // promise[ done | fail | progress | then | always | state | promise ]

      //è°ƒç”¨å†…éƒ¨è¾…åŠ©çš„promiseçš„promiseæ–¹æ³•ï¼ˆjQå‘çˆ¹ï¼Œèµ·åŒæ ·åå­—ï¼‰
      //æ‰©å±•deferredçš„then | done | fail | progressç­‰æ–¹æ³•
      promise.promise(deferred);

      //è¿™é‡Œä¸ºä»€ä¹ˆè¦åˆ†æˆpromiseè·Ÿdeferredä¸¤ä¸ªå¯¹è±¡å‘¢
      //å…¶å®žå°±æ˜¯å› ä¸ºä¸æƒ³æŠŠ[resolve | reject | notify]è¿™å‡ ä¸ªå‡½æ•°æš´éœ²å‡ºåŽ»
      //
      // Call given func if any
      //$.Deferred(func)æ ¼å¼ï¼Œåˆ™è‡ªåŠ¨æ‰§è¡Œä»»åŠ¡func
      //å¹¶ä¸”æŠŠå½“å‰ä»»åŠ¡çš„ä¸Šä¸‹æ–‡è·Ÿå‚æ•°è®¾ç½®æˆå½“å‰ç”Ÿæˆçš„deferredå®žä¾‹
      if (func) {
    ...
    // Deferred helper
    //æ³¨æ„åˆ°$.whenæ˜¯å¤šä»»åŠ¡çš„
    //å½“ä¸€ä¸ªä»»åŠ¡å¤±è´¥çš„æ—¶å€™ï¼Œä»£è¡¨æ•´ä¸ªéƒ½å¤±è´¥äº†ã€‚
    //ä»»åŠ¡æ˜¯Deferredå®žä¾‹ï¼Œæˆ‘æˆä¸ºå¼‚æ­¥ä»»åŠ¡
    //ä»»åŠ¡æ˜¯æ™®é€šfunctionæ—¶ï¼Œæˆ‘æˆä¸ºåŒæ­¥ä»»åŠ¡
    when: function(subordinate /* , ..., subordinateN */) {
      var i = 0,
        ...
        // æ²¡æœ‰å®Œæˆä»»åŠ¡çš„deferredå¯¹è±¡
        remaining =
          length !== 1 ||
